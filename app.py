...
with st.sidebar:
    ...
    with choose_model:
        with st.popover('model') :
             st.write('Gemini-2.5-flash :material/verified:')
...
cols = st.columns([8,1.2,1], vertical_alignment='center')
# with cols[0]:
...
if os.path.exists(FOLDER_PATH):
    if not os.listdir(FOLDER_PATH):
        st.warning(f"Dossier '{FOLDER_PATH}' est vide. Veuillez exécuter une commande d'agent pour générer des fichiers.")
